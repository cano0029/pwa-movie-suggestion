const APP = {
  ...
  addListeners() {
    ...
    document
      ...
      .addEventListener('submit', APP.navigateResultsPage)
  ...
  async navigateResultsPage(event) {
    event.preventDefault()
  ...
  async checkMovieStore(keyword) {
    ...
    transaction.oncomplete = () => {
      console.log(
        'Task of looking in movieStore for already saved movie results is complete'
      )
    }
    ...
    getRequest.onsuccess = (event) => {
      ...
      if (request.length === 0) {
        ...
        console.log(
          `Movie results found under "${keyword}" is already saved in movieStore`
        )
  ...
  async checkSuggestStore({ id, ref }) {
    ...
    transaction.oncomplete = () => {
      console.log(
        'Task of looking in suggestStore for the movie id is complete'
      )
    }
    ...
    getRequest.onsuccess = (event) => {
      ...
      if (request.length === 0) {
        ...
        console.log(
          `Recommended movies under genre id "${id}" is already saved in the suggestStore`
        )
        let movies = request[0].results
        APP.buildList(movies)
      }
  ...
  saveResults(movieResults) {
    ...
    request.onerror = (error) => {
      console.log(
        'Something went wrong in adding movie results to movieStore',
        error
      )
    }
  },
  ...
  saveSuggest(suggestResults) {
    ...
    request.onerror = (error) => {
      console.log(
        'Something went wrong in adding suggested to movieStore',
        error
      )
    }
  },
  ...
  openDB() {
    ...
    dbOpenRequest.addEventListener('upgradeneeded', (event) => {
      ...
      console.log(
        `movieDB Version Update: From version ${oldVersion} to version ${newVersion}`
      )

      if (!APP.db.objectStoreNames.contains('movieStore' && 'suggestStore')) {
        APP.moviedbStore = APP.db.createObjectStore('movieStore', {
          keyPath: 'keyword',
        })
